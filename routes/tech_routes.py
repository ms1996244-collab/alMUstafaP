...
# تعريف الـ Blueprint الخاص بقسم التكنولوجيا
...
def track_visitor():
    if request.endpoint and not request.endpoint.startswith('static') and request.endpoint not in ['admin.admin', 'admin.login', 'admin.logout']:
        raw_ip = request.headers.get('X-Forwarded-For', request.remote_addr)
        if raw_ip:
            clean_ip = raw_ip.split(',')[0].strip()
            ip_hash = hashlib.sha256(clean_ip.encode('utf-8')).hexdigest()
...
def sitemap():
    ...
    # يتم إضافة مقالات التداول هنا لاحقاً إذا رغبت، أو إنشاء Sitemap منفصل
